...
kernell_variance_component_ukbb<-function(gdsfile, groupfile, phenfile, ID_col, nullfile, outfile,
...
kernell_variance_component_chunk_ukbb<-function(gdsfile, groupfile, phenfile, ID_col, nullfile, outfile,
                                       AF.max=0.001, MAC.max=Inf, use.weights=FALSE,
                                       vc.test=c("Score", "Score.SPA"),
                                       test=c("SKAT", "SKATO", "SMMAT", "SKAT_SAIGEGENEplus", "ExtractKernelStatistics"),
                                       SAIGEGENEplus_collapse_threshold=10, weight.beta=c(1,1)){
        ...
        annot <- as.data.frame(annot)
        class(annot$pos) <- "numeric"
        annot<-annot[order(annot$pos),]
        unigenes<-unique(annot$group_id)
        ngroups<-length(unigenes)
        ngorder<-c(seq(1,ngroups,by=50),ngroups+1)

        for(ng in 1:(length(ngorder)-1)){

        # small chunk    
        anngenes<-unigenes[ngorder[ng]:(ngorder[ng+1]-1)]
        annot2<-annot[annot$group_id %in% anngenes,]

        # Grouping file; add weights if weights are selected
        weights.found<-FALSE
        if(use.weights){
                if(!"weight" %in% colnames(annot2)){
                        cat("\nWARNING: no column named 'weight' found in the grouping file; no weights will be applied.\n")
                        gr<-aggregateGRangesList(annot)
                }else{
                        #annot <- annot[,c("group_id", "chr", "pos", "ref", "alt", "weight")]
                        cat("\nuse.weights=T and 'weight' column found in grouping file; variant weights will be applied.\n")
                        gr<-aggregateGRangesList(annot1)
                        weights.found<-TRUE
                }
        }else{
                gr<-aggregateGRangesList(annot2)
        }
        ...
        outfile1<-gsub(".RData",paste0("_chunk",ng,".RData"),outfile)
        # Save results
        save(assoc,file=outfile1)
        seqClose(gds)
        }
                                       }


kernell_variance_component_by_chunk_ukbb<-function(gdsfile, groupfile, chunknum, phenfile, ID_col, nullfile, outfile,
                                       AF.max=0.001, MAC.max=Inf, use.weights=FALSE,
                                       vc.test=c("Score", "Score.SPA"),
                                       test=c("SKAT", "SKATO", "SMMAT", "SKAT_SAIGEGENEplus", "ExtractKernelStatistics"),
                                       SAIGEGENEplus_collapse_threshold=10, weight.beta=c(1,1)){
