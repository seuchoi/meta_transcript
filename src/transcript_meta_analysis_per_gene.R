...
transcript_meta_analysis_per_gene<-function(study_path_vector=study_path_vector, geneid=NULL,grouping_path_vector=grouping_path_vector,test=c("Burden", "SKAT","SKATO","SMMAT"), min_study_cmac=1,min_meta_cmac=20,rho=c(0, 0.1^2, 0.2^2, 0.3^2, 0.4^2, 0.5^2, 0.5, 1),use.anytranscript=T,combine.pval=T){
...
for (tt in 1:length(transcripts)){
  print(tt)
   ...
   for(i in c(1:n_studies)){
        ...
        out<-data.frame(group,transcript=transID,n_studies_effective=length(sv.list),n.site=length(unique(variant.id)),n.alt=sum(n.alts),stringsAsFactors=F)
