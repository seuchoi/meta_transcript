...
transcript_single_analysis_meta_nocovout_spa<-function(study_path=study_path,test=c("Burden"),
...
## ver 2
   groupings <- lapply(study_list, function(x) unique(x$results$genename) )
...
## start gene level
   for(gg in 1:length(groupings)){#(1)#
    ...
    transcripts <- lapply(study_list, function(x) unique(x$results[which(x$results$genename == group),"transcript"]) )
    ...
    if(length(transcripts)>0){ #(2)#
      for(tt in 1:length(transcripts)){ #(3)#
         ...
         variant.id <- n.alts <- n.sample.alt <- NULL
         n.site.list<-n.alt.list<-n.sample.alt.list<-V.list <- U.list <- NULL
      ...
      for(i in 1:n_studies){   #(4)#
          ...
          V.list<-c(V.list,trans_results$Burden_SPA.Score.Variance)
          n.site.list<-c(n.site.list,trans_results$n.site)
          n.alt.list<-c(n.alt.list,trans_results$n.alt)
          n.sample.alt.list<-c(n.sample.alt.list,trans_results$n.sample.alt)
        }else{
          ...
          n.sample.alt.list<-c(n.sample.alt.list,NA)
          }
        }
        ...
        out<-cbind(out,nalt)
        nsamplealt<-data.frame(t(n.sample.alt.list))
        colnames(nsamplealt)<-paste0("n.sample.alts.",1:length(n.sample.alt.list))
        out<-cbind(out,nsamplealt)
        rownames(out) <- group
