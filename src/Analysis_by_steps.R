...
# For binary outcome:
  filei       = "yourfile.tsv"   # file name of your main data set with all covariates and outcome of interest as columns
  ID_col      = "person_id"      # column name for sample ID  
  y           = "Disease_status" # column name for the outcome variable
  ...
  mtype       = "binomial"                    # binomial for binary outcome
  ...
  fit_nullmodel(phenofile=filei, 
                ID_col=ID_col, Outcome=y, IV_Rank_Norm=FALSE,  
...
# Example analysis for chromosome 18  
  ...
  groupfile = "Annotation_for_variants.RData"          # annotations for the variants (e.g., from VEP)
  phenfile  = "yourfile.tsv"                           # file name of your main data set with all covariates and outcome of interest as columns
  ID_col    = "person_id"                              # column name for sample ID  
  nullfile  = "nullmodel_results.RData"                # name of the output file from fit_nullmodel() function
...
# For chromosome 18  
  load("outfile_chr18.RData")  # output from kernell_variance_component_aou()
  res0  <- assoc$results
